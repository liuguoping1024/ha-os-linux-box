        ...
        if keyname:
            ...
            if item == '=':
                ...
                raise TomlDecodeError(
                    "Found invalid character in key name: '" + item +
                    "'. Try quoting the key name.", original, i)
    ...
    for idx, line in enumerate(s):
        ...
        if multikey:
            if multibackslash:
                ...
                multilinestr += line
            multibackslash = False
            if len(line) > 2 and (line[-1] == multilinestr[0] and line[-2] == multilinestr[0] and
            ...
            while i < len(groups):
                ...
                if len(groups[i]) > 0 and (groups[i][0] == '"' or groups[i][0] == "'"):
                    ...
                    if not _groupname_re.match(groups[i]):
                        raise TomlDecodeError(
                            "Invalid group name '" + groups[i] + "'. Try quoting it.", original,
                            pos)
                i += 1
            currentlevel = retval
                        ...
                        if group in implicitgroups:
                            ...
                            if arrayoftables:
                                raise TomlDecodeError(
                                    ...
                                    "table can't be an array", original, pos)
                        elif arrayoftables:
...
def _load_date(val):
    ...
    try:
        if len(val) == 10:
            ...
            d = datetime.datetime(int(val[:4]), int(val[5:7]), int(val[8:10]), int(val[11:13]),
...
class TomlDecoder(object):
    ...
    def load_inline_object(self, line, currentlevel, multikey=False, multibackslash=False):
        ...
        while len(candidate_groups) > 0:
            ...
            if ((value[0] == value[-1] and value[0] in ('"', "'")) or
                (value[0] in '-0123456789' or value in ('true', 'false') or
    ...
    def load_line(self, line, currentlevel, multikey, multibackslash):
        ...
        while len(
                pair[-1]) and (pair[-1][0] != ' ' and pair[-1][0] != '\t' and
                               pair[-1][0] != "'" and pair[-1][0] != '"' and pair[-1][0] != '[' and
                               pair[-1][0] != '{' and pair[-1] != 'true' and pair[-1] != 'false'):
            try:
                float(pair[-1])
                break
            except ValueError:
        ...
        if len(pair[1]) > 2 and ((pair[1][0] == '"' or pair[1][0] == "'") and
                                 pair[1][1] == pair[1][0] and pair[1][2] == pair[1][0] and
                                 not (len(pair[1]) > 5 and pair[1][-1] == pair[1][0] and
                                      pair[1][-2] == pair[1][0] and pair[1][-3] == pair[1][0])):
            k = len(pair[1]) - 1
            ...
            if strarray:
                while b < len(a) - 1:
                    ...
                    while (not self.bounded_string(ab) or
                           (len(ab) > 2 and ab[0] == ab[1] == ab[2] and ab[-2] != ab[0] and
                            ab[-3] != ab[0])):
                        a[b] = a[b] + ',' + a[b + 1]
                        ab = a[b].strip()
                        if b < len(a) - 2:
