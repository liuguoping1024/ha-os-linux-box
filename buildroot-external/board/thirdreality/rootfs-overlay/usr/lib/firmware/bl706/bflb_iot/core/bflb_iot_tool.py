...
class BflbIotTool(object):
    ...
    def __init__(self, chipname, chiptype, outdir):
        ...
        if chiptype == "bl60x":
            self.bh_cfg_file_org = os.path.join(chip_path, chipname,
                                           "efuse_bootheader/efuse_bootheader_cfg_dp.conf")
        else:
            self.bh_cfg_file_org = os.path.join(chip_path, chipname,
                                           "efuse_bootheader/efuse_bootheader_cfg.conf")
        self.bh_cfg_file = os.path.join(self.outdir, 
...
#             shutil.rmtree(ota_path)
        ...
        if len(fw_ota_bin) > pt_parcel[pt_fwlen_name]:
            ...
            raise Exception(
                "Error: fw bin size is overflow with partition table, don't create ota bin")
            return False
...
#                 security_save_efuse = (cfg.get("EFUSE_CFG", "security_save") == "true")
    ...
    def bl60x_mfg_flasher_eflash_loader_cfg(self,
        ...
        if boot2 is True:
            ...
            bflb_utils.copyfile(os.path.join(self.outdir, "bootinfo.bin"), os.path.join(self.outdir, "bootinfo_boot2.bin"))
            d_files.append(os.path.join(self.outdir, "bootinfo_boot2.bin"))
            d_addrs.append("00000000")
                    ...
                    if name + '_addr' in pt_parcel:
                        ...
                        if bind_bootinfo is True:
                            if name == "fw":
                                ...
                                bootinfo_filedata = bl_get_file_data([os.path.join(self.outdir, "bootinfo_" + name + ".bin")])[0]
                                filedata = bl_get_file_data([os.path.join(self.outdir, "img_" + name + ".bin")])[0]
                                if bootinfo_filedata[0:4] == filedata[0:4]:
                                    whole_img_data = filedata
                                else:
                                    whole_img_len = offset + os.path.getsize(os.path.join(self.outdir, "img_" + name + ".bin"))
                                    whole_img_data = bl_create_flash_default_data(whole_img_len)
                                    whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                                    whole_img_data[offset:offset + len(filedata)] = filedata
                                fp = open(whole_img, 'wb+')
                            ...
                            filedata = bl_get_file_data(
                                [os.path.join(self.outdir, "img_" + name + ".bin")])[0]
                            if bootinfo_filedata[0:4] == filedata[0:4]:
...
#                 return bflb_utils.errorcode_msg()
            ...
            if 'boot2_len' in pt_parcel and pt_parcel['boot2_len'] == 0:
                ...
                if boot2 is not None and len(boot2) > 0:
                    ...
                    self.bl60x_image_gen(chipname, chiptype, cpu_type, "boot2", boot2,
                                         ...
                                         publickey_str=config["param"]["publickey"], encrypt=self.encrypt, sign=self.sign)
                    boot2_d = True
            elif 'boot2_header' in pt_parcel and not pt_parcel['boot2_header']:
                                    ...
                                    if chiptype == "bl616":
                                        ...
                                        if cfg_flasher.has_option("THIRD_CFG", "amazon_header_enable"):
                                            if (cfg_flasher.get("THIRD_CFG", "amazon_header_enable") == "true"):
                                                ...
                                                shutil.copyfile(os.path.join(self.outdir, "img.bin"), out_encrypt_file)
    ...
    def flasher_download_thread(self, chipname, chiptype, act, config, callback=None):
        ...
        if act != "build" and act != "download" and act != "erase" and act != "read":
        ...
        try:
            ...
            if act == "build":
                ...
                error = self.eflash_loader_t.efuse_flash_loader(args, self.eflash_loader_cfg,
                                                                self.eflash_loader_bin, callback, None,
                                                                None)
                bflb_utils.printf(error)
            ...
            # generation partition table/boot2/fw bin files and rf files
                ...
                if "erase_only" in config[
                    ...
                    if (self.encrypt is True or self.sign is True) and (chiptype == "bl602" or chiptype == "bl702"):
                        ...
                        ret, encrypted_data = bflb_img_create.encrypt_loader_bin(chiptype, \
                        self.eflash_loader_bin, sign, encrypt, key, iv, publickey, privatekey)
                        if ret == True:
                            ...
                            z.write(os.path.join(chip_path, chipname, "eflash_loader", eflash_loader_bin), os.path.join(chipname, "eflash_loader", eflash_loader_bin))
                            z.close()
                    if error is True:
                ...
                args = parser_eflash.parse_args(options)
                error = self.eflash_loader_t.efuse_flash_loader(args, self.eflash_loader_cfg,
                                                                self.eflash_loader_bin, callback,
