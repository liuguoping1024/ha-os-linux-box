...
def firmware_post_proc_do_encrypt(data_bytearray,aeskey_hexstr,aesiv_hexstr,xts_mode,privatekey, publickey,imgfile):
    flash_img=1
    bootcfg_start=8+4+84+4+4+16+4
    xip_addr=bflb_utils.bytearray_reverse(data_bytearray[0xB4:0xB8])
    segheader = bytearray(0)
    if bflb_utils.bytearray_to_int(xip_addr)!=0x00 and \
       bflb_utils.bytearray_to_int(xip_addr)!=0x80000000:
    ...
    if flash_img:
        ...
        boot_data[0xB4:0xB4+4]=segheader[0:4]
        if (boot_data[0xB7]&0x0f)<=0x01:
...
def firmware_post_proc_update_clock_crc(image_data):
    ...
    crcarray = bflb_utils.get_crc32_bytearray(image_data[clockcfg_start+4:clockcfg_start+20])
    image_data[clockcfg_start+20:clockcfg_start+20+4] = crcarray
...
def firmware_post_proc_get_hash_ignore(image_data):
    bootcfg_start=(4+4)+(4+84+4)+(4+16+4)
...
def firmware_post_proc_get_image_offset(image_data):
    cpucfg_start=(4+4)+(4+84+4)+(4+16+4)
    return  ((image_data[cpucfg_start + 4])+
