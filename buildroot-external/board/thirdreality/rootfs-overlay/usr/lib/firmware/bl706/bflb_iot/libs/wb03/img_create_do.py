...
def img_create_get_img_offset(bootheader_data):
    return bflb_utils.bytearray_to_int(bflb_utils.bytearray_reverse(
           bootheader_data[bootcfg_start + 4 : bootcfg_start + 8]))
...
def img_create_update_bootheader_if(bootheader_data, hash, seg_cnt):
    ...
    if (bootheader_data[bootcfg_start + 2] & 0x1) == 1:
        ...
        hd_crcarray = bflb_utils.get_crc32_bytearray(bootheader_data[custom_cfg_len:header_len -
                                                                     4])
...
# update custom boot header info
def img_create_update_custom_bootheader(bootheader_data, seg_cnt, hash, signature, pk_data,
                                        aesiv_data):
    # update custom vendor boot len
    bootheader_data[20:24] = bflb_utils.int_to_4bytearray_l(seg_cnt)

    # update custom header hash/signature/pk/aesiv
    if hash != bytearray(0):
...
def img_creat_process(group_type, flash_img, cfg, security=False):
    ...
    if flash_img == 1:
        ...
        if bootheader_data[25 + 208:26 + 208] == b'\xff':
            flashCfgList, flashCfgTable, flashCfgListLen = create_flashcfg_table(flashCfgAddr)

        # only flash boot need custom header
        # update custom boot header
        bootheader_data = img_create_update_custom_bootheader(bootheader_data, seg_cnt, hash,
                                                              signature, pk_data, aesiv_data)
    ...
    if flash_img == 1:
        ...
        if encrypt != 0:
            ...
            img_update_efuse_fun(cfg, sign, pk_hash, 0, None, \
                                 key_sel, bytearray(32), security)
    return "OK", data_tohash
